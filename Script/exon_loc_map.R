...
Find_id_loc = function(ETable, TRANS.ID, EXON.ID) {
  ...
  for(i in 1:length(TRANS.ID)){
    ...
    if (exon.df$start[head(UTR.3,1)] <= cds.box[1] && exon.df$end[tail(UTR.5,1)] >= cds.box[2]) {
      #translation start inside the last exon
      cds.start = NULL
      cds.end   = cds.box[2] - cds.box[1] + 1 
    } else { 
      #translation start before the last exon
